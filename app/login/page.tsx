...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                let errorMessage = data.error || data.detail || (isUz ? "Login yoki parol noto'g'ri" : "ユーザー名またはパスワードが正しくありません");
                throw new Error(errorMessage);
            }

            // CROSS-NATIONALITY CHECK
            // Ensure the user's registered nationality matches the current site language section
            if (data.user.nationality && data.user.nationality !== nationality) {
                throw new Error(
                    isUz
                        ? "Siz boshqa davlat bo'limiga kira olmaysiz. Iltimos, o'z tilingizga mos bo'limdan kiring."
                        : "他の国のセクションにはログインできません。適切な言語セクションからログインしてください。"
                );
            }
                ...
                <div className="bg-white/70 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] p-8 border border-white/40 animate-in slide-in-from-bottom-8 duration-700">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="text-right">
                            <Link href="/forgot-password" className={`text-sm ${isUz ? "text-blue-600" : "text-rose-600"} hover:underline font-bold`}>
                                {t.forgotPassword}
                            </Link>
                        </div>
                    ...
                    <div className="mt-8 text-center border-t border-slate-100 pt-6">
                        <p className="text-slate-600 font-medium">
                            {t.noAccount}{" "}
                            <Link href="/register" className={`${isUz ? "text-blue-600" : "text-rose-600"} hover:underline font-bold transition-all`}>
                                {t.register}
                            </Link>
                        </p>
                    </div>
